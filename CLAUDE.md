...
- **현재 수치** (금감원 실데이터 202608 + 법령 7종 1,025조문 적재 기준): 검색
  ...
  12% / 78% / 100%; KR-FinReg 이식 71문항 78% / 100% / 100%; **Bench v2.2
  880문항 75% / 99% / 99%** (집계 축 벡터 30~31%·계산 축 58~63%로 하락, 그래프 100%).
  v2.2 계산 추론 축 136문항: 세전/세후 이자·이자 차액(Decimal 산출, 독립 재검산
  통과)·연령×경로 결합 판정 — 답변 계층에서 다단계 수치 추론 변별력 제공. 환각유도 컨텍스트: 벡터 2,020자 vs 그래프 0자.
...
- **시나리오 매트릭스 (Bench v2 880문항, R@4K 예산)**: S1 청크 82% / S6 LPG 85% /
  S7 원시 온톨로지 71% (컨텍스트 114K자 — 과잉) / S9 온톨로지+CE 87% (53K자).
  무제한이면 그래프 계열 전부 99%. CE 규칙은 gold 무관 구조 규칙만 사용
  (URI 참조 제거, 개체→속성→클래스 우선순위, 사용처 미리보기 160자). "시드 우선"
  재정렬은 전체 셋에서 순손실(87→85%)이라 제거함 — 표본(각 템플릿 앞부분)으로
  판단하지 말 것.
